...
if __name__ == "__main__":
    ...
    def safe_put(txn, key, filepath):
        """Store file in LMDB if it exists and not already stored."""
        if txn.get(key) is not None:
            return  # Already stored
        if not os.path.exists(filepath):
            return  # File doesn't exist
        txn.put(key, open(filepath, 'rb').read())

    # Convert to LMDB
    lmdb_path = "/home/ulrik/NTNU/master/sim/simlingo/database/lmdb_dataset"
    with lmdb.open(lmdb_path, map_size=500*1024**3) as env:
        with env.begin(write=True) as txn:
            ...
            for idx in tqdm(range(len(dataset)), desc="Measurements/Commentary/QA"):
                ...
                for i in range(dataset.hist_len + dataset.pred_len):
                    ...
                    # measurement_path.replace('measurements', 'commentary').replace('data/', 'commentary/')
                    commentary_path = meas_path.replace('measurements', 'commentary').replace('data/', 'commentary/')
                    safe_put(txn, commentary_path.encode(), commentary_path)

                    # QA/VQA file (derived from measurement path)
                    # measurement_path.replace('measurements', 'vqa').replace('data/', 'drivelm/')
                    qa_path = meas_path.replace('measurements', 'vqa').replace('data/', 'drivelm/')
                    safe_put(txn, qa_path.encode(), qa_path)
